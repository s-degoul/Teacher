...
if (isset ($_POST['valid_password_forgotten'])) {
	...
	if ($nb_response == 1) {
		$new_password = createRandomString (15);
		...
		$mail_content = _("Vous recevez ce mail parce que vous avez dit avoir oublié votre mot de passe pour accéder au site TEACHER.")."\r\n"
						...
						._("Si vous n'êtes pas à l'origine de ce mail, prière de vous connecter de tout urgence à TEACHER avec ce nouveau mot de passe et de le modifier,
							et de contacter l'administrateur du site").' ( '.MAIL_ADMIN.' )';
				
		mail ($user['user_mail'], $mail_subject, $mail_content);

		
		$messages['info'][] = _("un mail a été envoyé (à l'adresse mail de votre profil) avec un nouveau mot de passe");
	}
	else {
